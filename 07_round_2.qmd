...
The number of each type of questions will vary between projects.

The code will deal automatically with type 1 questions.

You will very likely need to recode manually type 2 and 3 questions.

-   [ ] Update the following files in `./analysis/dft2/` so they correspond to your project:
...
This code will create the word document ("generic report") inserting the introductory texts (updated in @sec-update-introductory-texts-round2) and the results tables (created in @sec-prepare-tables-without-participant-id-round2), based on a reference word template.

It automatically calls `./code/dft2/01c_dft2_define_cols.R.`

It will then automatically call one `*.Rmd` per section, which themselves will call two codes:
...
This code will create the word document for each participant ("individualized report"), based on a word template, by inserting the email of the participant, the introductory texts (updated in @sec-update-introductory-texts-round2), as well as the group result tables with individual answers for each statement.
...
The code `./analysis/dft2/dft2_report_per_participant.Rmd` (automatically called within `./code/dft2/05_dft2_to_render_individual_reports.R`) functions as an iterative loop and creates the individualized result report for each participant by calling automatically:
...
It will then call one `\*.Rmd` file per section (same files as for the generic report), which themselves call two codes:
