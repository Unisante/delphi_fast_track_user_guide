...
We have created a *fast-track Delphi* REDCap "skeleton" project including all survey parameters and structured exemplar items for type 1, 2 and 3 questions. These parameters are stored in an `*.xml` file available in the folder `./_REDCap/`. There are two `*.xml` files - one for the 'round 2' REDCap project and one for the 'round 3' REDCap project. 
